import {
  ...
  export const getDevolucionesPendientes = async (movimientoId) => {
    ...
    const [result] = await db.promise().query(query, [movimientoId]);
  ...
  export const createMovimiento = async (movimiento, imagen) => {
  ...
  export const updateMovimiento = async (id, movimiento, imagen) => {
    if (!id) throw new Error("ID es obligatorio");
