...
export const findUserByUsername = async (nombre_usuario) => {
  ...
  export const updateUserApprovalStatus = async (userId, status) => {
    ...
    try {
      const [result] = await db.promise().execute(query, [status, userId]);
      return result.affectedRows > 0;
    } catch (error) {
...
export const savePasswordResetToken = async (userId, token, tokenExpires, code = null, codeExpires = null) => {
  const query = `
    ...
    WHERE ID_Usuario = ?
  `;
  await db.promise().execute(query, [token, tokenExpires, code, codeExpires, userId]);
  return true;
};
...
export const getUserByResetToken = async (token) => {
  ...
  export const updateUserPassword = async (userId, hashedPassword) => {
    ...
    try {
      const [result] = await db.promise().execute(query, [hashedPassword, userId]);
...
export const deleteAdmi = async (ID_Usuario) => {
  try {
    ...
    await db.promise().execute("DELETE FROM imagen WHERE ID_Usuario = ?", [ID_Usuario]);
