...
export const deleteProducto = async (id) => {
  try {
    ...
    if (!puedeEliminar) {
      throw new Error("El producto tiene datos relacionados importantes y no puede ser eliminado");
    }

    await eliminarImagenesDeProducto(id);
    
    await db.promise().execute("DELETE FROM movimiento WHERE ProductoID_Producto = ?", [id]);
    const [result] = await db.promise().execute("DELETE FROM producto WHERE ID_PRODUCTO = ?", [id]);
