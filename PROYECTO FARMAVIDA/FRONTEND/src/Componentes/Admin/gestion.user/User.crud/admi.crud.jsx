...
function Admicrud() {
  ...
  const getAdmicrud = async () => {
    const respuesta = await axios.get("http://localhost:4000/api/obtener/administrador");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (operacion === 1) {
        await axios.post("http://localhost:4000/api/registrar/administrador", Admicrud);
      } else if (operacion === 2) {
        await axios.put(
          `http://localhost:4000/api/actualizar/administrador/${ID_Usuario}`,
          Admicrud
        );
      }
      getAdmicrud();
  ...
  const deleteAdmicrud = async (ID_Usuario, nombre) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://localhost:4000/api/eliminar/administrador/${ID_Usuario}`);
          getAdmicrud(); 
