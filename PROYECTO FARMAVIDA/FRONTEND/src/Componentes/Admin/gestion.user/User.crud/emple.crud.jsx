...
function Emplecrud() {
  ...
  const getEmpleados = async () => {
    try {
      const respuesta = await axios.get(
        "http://localhost:4000/api/obtener/empleado"
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (operacion === 1) {
        await axios.post(
          "http://localhost:4000/api/registrar/empleado",
          empleado
        );
      } else {
        await axios.put(
          `http://localhost:4000/api/actualizar/empleado/${ID_Usuario}`,
          empleado
        );
      }
      getEmpleados();
  ...
  const deleteEmpleado = async (ID_Usuario, nombre) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `http://localhost:4000/api/eliminar/empleado/${ID_Usuario}`
          );
          getEmpleados();
