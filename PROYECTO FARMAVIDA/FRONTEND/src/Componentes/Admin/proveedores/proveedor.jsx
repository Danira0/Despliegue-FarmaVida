...
function Proveedor() {
  ...
  const getProveedores = async () => {
    const respuesta = await axios.get(
      "http://localhost:4000/api/obtener/proveedores"
    );
  ...
  const openModal = (
    ...
    setImagen(null);
    setImagenPreview(
      imagen_producto ? `http://localhost:4000/${imagen_producto}` : ""
    );
    setOperacion(op);
    if (op === 1) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (operacion === 1) {
        await axios.post(
          "http://localhost:4000/api/registrar/proveedores",
          formData,
          {
            headers: {
        ...
        await axios.put(
          `http://localhost:4000/api/actualizar/proveedores/${ID_Laboratorio}`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
  ...
  const deleteProveedor = async (ID_Laboratorio, nombre) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `http://localhost:4000/api/eliminar/proveedores/${ID_Laboratorio}`
          );
          getProveedores();
                            ...
                            <tr key={proveedor.ID_Laboratorio}>
                              ...
                              <td>
                                {proveedor.imagen_producto && (
                                  <img
                                    src={`http://localhost:4000/${proveedor.imagen_producto}`}
