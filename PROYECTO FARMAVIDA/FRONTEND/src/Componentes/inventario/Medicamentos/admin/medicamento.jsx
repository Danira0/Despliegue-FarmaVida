  ...
  const getNombresLaboratorios = async () => {
    try {
      const response = await axios.get(
        "http://localhost:4000/api/obtener/laboratorios/nombres"
      );
    ...
    try {
      ...
      if (operacion === 1) {
        response = await axios.post(
          "http://localhost:4000/api/registrar/productos",
          formData,
          {
            headers: {
        ...
        response = await axios.put(
          `http://localhost:4000/api/actualizar/productos/${ID_PRODUCTO}`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
  ...
  const deleteProducto = async (ID_PRODUCTO, nombre) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `http://localhost:4000/api/eliminar/productos/${ID_PRODUCTO}`
          );
